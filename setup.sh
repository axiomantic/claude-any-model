...
def run_model_configuration():
    ...
    # Build the full allowed set: canonical names + every alias that maps to a configured tier (+ [1m] context suffixes)
    raw_aliases = {alias for alias, canon in TIER_ALIASES.items() if canon in canonical_names} | canonical_names
    allowed_models = sorted(raw_aliases | {f"{a}[1m]" for a in raw_aliases} | {f"{a}-1m" for a in raw_aliases})
...
class StrictModelGuardrail(CustomLogger):
    async def async_pre_call_hook(self, user_api_key_dict, cache, data, call_type, **kwargs):
        ...
        if requested_model:
            # Strip context window suffixes like [1m], [200k], etc. that Claude Desktop appends
            base_model = re.sub(r'\\[.*?\\]$', '', requested_model).strip()
            canonical = ALIAS_MAP.get(base_model, ALIAS_MAP.get(requested_model, base_model))
...
"""
    ...
    for s in selections:
        ...
        all_tier_aliases = set(aliases)
        for a in aliases:
            all_tier_aliases.add(f"{a}[1m]")
            all_tier_aliases.add(f"{a}-1m")

        yaml_lines.append(f"  # ── {s['tier'].capitalize()} tier → {s['label']}")
        for alias in sorted(all_tier_aliases):
